...
export function resolveVisibility(skills, plugins, settingsPath) {
  ...
  try {
    ...
    for (const skill of skills) {
      if (skill.origin !== 'plugin' && skill.visibility === 'on') {
        skill.visibility = 'on';
  ...
  for (const skill of skills) {
    ...
    // Check by commandName and also by the directory name (skill id suffix)
    const dirName = skill.commandName; // For standalone skills, commandName IS the dir name
    const override = skillOverrides[dirName] || skillOverrides[skill.commandName];
